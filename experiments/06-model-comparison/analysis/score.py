...
run_evaluation.py, producing one row per (model, example) with a score in
[0, 1] and a failure-category detail string.

Directly adapted from Week 5's score.py — same scorer, keyed by `model` instead
of `quant_level`.
"""
...
RAW_PATH = REPO_ROOT / "results" / "raw" / "06-model-comparison" / "raw_outputs.jsonl"
